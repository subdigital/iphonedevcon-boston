...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemOrganize 
                                                                                            target:self 
                                                                                            action:@selector(downloadAvatars:)] autorelease];
    
}

- (void)downloadAvatars:(id)sender {
    ...
    for (NSString *avatar in [self avatars]) {
        NSURL *url = [NSURL URLWithString:[[avatarInfo objectForKey:@"BaseURL"] stringByAppendingString:avatar]];
        ASIHTTPRequest *avatarRequest = [ASIHTTPRequest requestWithURL:url];
        
        NSString *docsDir = nil;
        NSArray *directories = NSSearchPathForDirectoriesInDomains(NSUserDirectory, NSUserDomainMask, YES);
        if ([directories count] > 0)
            docsDir = [directories objectAtIndex:0];
        
        NSString *localPath = [docsDir stringByAppendingPathComponent:avatar];
        [avatarRequest setDownloadDestinationPath:localPath];
        
        [networkQueue addOperation:avatarRequest];
    }
